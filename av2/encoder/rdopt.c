  ...
  if (is_comp_pred) {
    ...
    if (this_mode == NEW_NEWMV || this_mode == NEW_NEWMV_OPTFLOW) {
      if (reuse_comp_mv_for_opfl(cm, x, args, cur_mv, rate_mv)) {
        ...
        if (cur_mv[0].as_int == INVALID_MV || cur_mv[1].as_int == INVALID_MV) {
          return INT64_MAX;
        }
        int_mv start_mv = { 0 };
      ...
      if (cm->seq_params.enable_adaptive_mvd) {
        ...
        av2_compound_single_motion_search_interinter(cpi, x, bsize, cur_mv,
                                                     NULL, 0, rate_mv, 1);
        if (cur_mv[1].as_int == INVALID_MV) return INT64_MAX;
        save_comp_mv_search_stat(x, args, cur_mv, start_mv);
      } else {
      ...
      if (cm->seq_params.enable_adaptive_mvd) {
        av2_compound_single_motion_search_interinter(cpi, x, bsize, cur_mv,
                                                     NULL, 0, rate_mv, 0);
        if (cur_mv[0].as_int == INVALID_MV) return INT64_MAX;
        int_mv start_mv = { 0 };
        save_comp_mv_search_stat(x, args, cur_mv, start_mv);
      } else {
        // avmenc3
...
static int eval_simple_translation(
    const AV2_COMP *const cpi, MACROBLOCK *const x, MACROBLOCKD *const xd,
    const AV2_COMMON *const cm, MB_MODE_INFO *mbmi, BLOCK_SIZE bsize,
    const BUFFER_SET *orig_dst, int rate2_nocoeff, int rate_mv0, int mi_row,
    int mi_col, int *tmp_rate2, int *tmp_rate_mv) {
...
static int eval_interintra(const AV2_COMP *const cpi, MACROBLOCK *const x,
                           BLOCK_SIZE bsize, MB_MODE_INFO *mbmi,
                           HandleInterModeArgs *const args, int64_t ref_best_rd,
                           const BUFFER_SET *orig_dst, int *tmp_rate_mv,
                           int *tmp_rate2) {
  const int ret = av2_handle_inter_intra_mode(
      cpi, x, bsize, mbmi, args, ref_best_rd, tmp_rate_mv, tmp_rate2, orig_dst);
  if (ret < 0) return 0;
  assert(mbmi->motion_mode == INTERINTRA);
  return 1;
}

static int eval_warp_causal(
    const AV2_COMP *const cpi, MACROBLOCK *const x, MACROBLOCKD *const xd,
    const AV2_COMMON *const cm, MB_MODE_INFO *mbmi, MB_MODE_INFO_EXT *mbmi_ext,
    BLOCK_SIZE bsize, int mi_row, int mi_col, const BUFFER_SET *orig_dst,
    PREDICTION_MODE this_mode, int rate2_nocoeff, int rate_mv0,
    int_mv *previous_mvs, int total_samples0, int total_samples1, int *pts0,
    int *pts_inref0, int *pts1, int *pts_inref1, uint8_t org_warp_inter_intra,
    HandleInterModeArgs *const args, int64_t ref_best_rd, int *tmp_rate2,
    int *tmp_rate_mv) {
  int pts[SAMPLES_ARRAY_SIZE], pts_inref[SAMPLES_ARRAY_SIZE];
  mbmi->wm_params[0].wmtype = DEFAULT_WMTYPE;
  mbmi->wm_params[1].wmtype = DEFAULT_WMTYPE;

  mbmi->warp_inter_intra = 0;  // initialize to 0 so that warp search can-be
                               // performed without inter-intra
  ...
  if (mbmi->mode == WARPMV) {
    ...
    if (!is_warp_candidate_inside_of_frame(cm, xd, mbmi->mv[0])) return 0;
    assert(mbmi->pb_mv_precision == mbmi->max_mv_precision);
    ...
    if (mbmi->warpmv_with_mvd_flag) {
      if (previous_mvs[mbmi->warp_ref_idx].as_int == INVALID_MV) {
        ...
        {
          if (mbmi->use_amvd) {
            av2_amvd_single_motion_search(cpi, x, bsize, &warp_ref_mv.as_mv,
                                          &tmp_trans_ratemv, 0);
          } else {
            av2_single_motion_search(cpi, x, bsize, 0, &tmp_trans_ratemv, 16,
                                     NULL, &mbmi->mv[0], &warp_ref_mv);
          }
        }
  ...
  l0_invalid = mbmi->wm_params[0].invalid;
  ...
  if (has_second_ref(mbmi)) {
    ...
    l1_invalid = mbmi->wm_params[1].invalid;
  }
  ...
  if (!l0_invalid && (!has_second_ref(mbmi) || !l1_invalid)) {
    if ((((this_mode == WARP_NEWMV || this_mode == NEW_NEWMV) && !l0_invalid) &&
      ...
      const int_mv mv0 = mbmi->mode == WARPMV ? warp_ref_mv : mbmi->mv[0];
      const int_mv ref_mv = mbmi->warpmv_with_mvd_flag ? warp_ref_mv :
...
static int eval_warp_delta(const AV2_COMP *const cpi, MACROBLOCK *const x,
                           MACROBLOCKD *const xd, const AV2_COMMON *const cm,
                           MB_MODE_INFO *mbmi, MB_MODE_INFO_EXT *mbmi_ext,
                           BLOCK_SIZE bsize, int mi_row, int mi_col,
                           const BUFFER_SET *orig_dst, int rate2_nocoeff,
                           int rate_mv0, int_mv *previous_mvs,
                           warp_mode_info_array *prev_best_models,
                           uint8_t org_warp_inter_intra,
                           HandleInterModeArgs *const args, int64_t ref_best_rd,
                           int *tmp_rate2, int *tmp_rate_mv) {
  ...
  if (!allow_warp_parameter_signaling(cm, mbmi)) {
    if (mbmi->warp_precision_idx) return 0;
...
static int eval_warp_extend(const AV2_COMP *const cpi, MACROBLOCK *const x,
                            MACROBLOCKD *const xd, const AV2_COMMON *const cm,
                            MB_MODE_INFO *mbmi, MB_MODE_INFO_EXT *mbmi_ext,
                            BLOCK_SIZE bsize, int mi_row, int mi_col,
                            const BUFFER_SET *orig_dst, int rate2_nocoeff,
                            int rate_mv0, uint8_t org_warp_inter_intra,
                            HandleInterModeArgs *const args,
                            int64_t ref_best_rd, int *tmp_rate2,
                            int *tmp_rate_mv) {
  if (mbmi->mode == WARP_NEWMV &&
      mbmi->pb_mv_precision < MV_PRECISION_ONE_PEL) {
    // Don't bother with warp modes for MV precisions >1px
  ...
  if (mbmi->mode == NEARMV) {
    ...
    if (mv0.as_int != mbmi->mv[0].as_int) {
      // Keep the refined MV and WM parameters.
      *tmp_rate_mv =
          av2_mv_bit_cost(&mbmi->mv[0].as_mv, &ref_mv.as_mv,
...
static int estimate_or_search_tx(
  ...
  if (!do_tx_search) {
    ...
    if (enable_tx_prune) {
      ...
      if (prune_motion_mode(est_rd, top_motion_mode_model_rd)) return 0;
    }

    // Do transform search
    if (!av2_txfm_search(cpi, x, bsize, rd_stats, rd_stats_y, rd_stats_uv,
                         rd_stats->rate, enable_tx_prune ? 0 : 1,
                         (*ref_best_rd))) {
          ...
          for (int warp_precision_idx = 0;
            ...
            *mbmi = base_mbmi;
            ...
            if (mbmi->motion_mode == SIMPLE_TRANSLATION) {
              if (!eval_simple_translation(
                      cpi, x, xd, cm, mbmi, bsize, orig_dst, rate2_nocoeff,
                      rate_mv0, mi_row, mi_col, &tmp_rate2, &tmp_rate_mv)) {
                continue;
              }
            } else if (mbmi->motion_mode == WARP_CAUSAL) {
              if (!eval_warp_causal(
                      cpi, x, xd, cm, mbmi, mbmi_ext, bsize, mi_row, mi_col,
                      orig_dst, this_mode, rate2_nocoeff, rate_mv0,
                      previous_mvs, total_samples0, total_samples1, pts0,
                      pts_inref0, pts1, pts_inref1, org_warp_inter_intra, args,
                      ref_best_rd, &tmp_rate2, &tmp_rate_mv)) {
                continue;
              }
            } else if (mbmi->motion_mode == INTERINTRA) {
              if (!eval_interintra(cpi, x, bsize, mbmi, args, ref_best_rd,
                                   orig_dst, &tmp_rate_mv, &tmp_rate2)) {
                continue;
              }
            } else if (mbmi->motion_mode == WARP_DELTA) {
              if (!eval_warp_delta(cpi, x, xd, cm, mbmi, mbmi_ext, bsize,
                                   mi_row, mi_col, orig_dst, rate2_nocoeff,
                                   rate_mv0, previous_mvs, &prev_best_models,
                                   org_warp_inter_intra, args, ref_best_rd,
                                   &tmp_rate2, &tmp_rate_mv)) {
                continue;
              }
            } else if (mbmi->motion_mode == WARP_EXTEND) {
              if (!eval_warp_extend(cpi, x, xd, cm, mbmi, mbmi_ext, bsize,
                                    mi_row, mi_col, orig_dst, rate2_nocoeff,
                                    rate_mv0, org_warp_inter_intra, args,
                                    ref_best_rd, &tmp_rate2, &tmp_rate_mv)) {
                continue;
              }
            }
...
static INLINE bool mask_check_bit(uint64_t mask, int index) {
...
static int ref_mv_idx_to_search(AV2_COMP *const cpi, MACROBLOCK *x,
  ...
  int ref_set_local[2] = { ref_set[0], ref_set[1] };
  if (!has_second_drl(mbmi)) ref_set_local[1] = 1;

  // Only search indices if they have some chance of being good.
  int good_indices = 0;

  int ref_mv_idx[2];
  for (ref_mv_idx[1] = 0; ref_mv_idx[1] < ref_set_local[1]; ++ref_mv_idx[1]) {
    for (ref_mv_idx[0] = 0; ref_mv_idx[0] < ref_set_local[0]; ++ref_mv_idx[0]) {
...
static AVM_INLINE void init_predictor_iteration_context(
    ...
    int base_rate, const int_mv *cur_mv, int rate_mv, MB_MODE_INFO base_mbmi,
    int refinemv_loop, int num_planes, int skip_motion_mode_value) {
  it_ctx->bsize = bsize;
...
typedef struct {
  ...
  int *best_cwp_idxs;
  int64_t *ref_best_rd;
  int (*best_ref_mv_idx)[2];

  inter_mode_info (*mode_info)[BAWP_OPTION_CNT][NUM_MV_PRECISIONS][2]
                              [MAX_REF_MV_SEARCH * MAX_REF_MV_SEARCH];
  int_mv (
      *save_mv)[2][NUM_MV_PRECISIONS][MAX_REF_MV_SEARCH * MAX_REF_MV_SEARCH][2];
} PredictorSearchState;

static AVM_INLINE void init_predictor_search_state(
    PredictorSearchState *search_state, int64_t *best_rd,
    RD_STATS *best_rd_stats, RD_STATS *best_rd_stats_y,
    RD_STATS *best_rd_stats_uv, MB_MODE_INFO *best_mbmi, SUBMB_INFO *best_submi,
    int *best_xskip_txfm, uint8_t (*best_blk_skip)[MAX_MIB_SIZE * MAX_MIB_SIZE],
    TX_TYPE *best_tx_type_map, CctxType *best_cctx_type_map,
    MvSubpelPrecision *best_precision_so_far, int *best_precision_dx_so_far,
    int64_t *best_precision_rd_so_far, int64_t *best_cwp_costs,
    int *best_cwp_idxs, int64_t *ref_best_rd, int (*best_ref_mv_idx)[2],
    inter_mode_info (*mode_info)[BAWP_OPTION_CNT][NUM_MV_PRECISIONS][2]
                                [MAX_REF_MV_SEARCH * MAX_REF_MV_SEARCH],
    int_mv (*save_mv)[2][NUM_MV_PRECISIONS]
                     [MAX_REF_MV_SEARCH * MAX_REF_MV_SEARCH][2]) {
  ...
  search_state->best_submi = best_submi;
  ...
  search_state->best_precision_so_far = best_precision_so_far;
  search_state->best_precision_dx_so_far = best_precision_dx_so_far;
  search_state->best_precision_rd_so_far = best_precision_rd_so_far;
...
static AVM_INLINE void update_predictor_search_state(
  ...
  if (tmp_rd < *search_state->best_rd) {
    ...
    *search_state->best_mbmi = *mbmi;
    if (is_warp_mode(mbmi->motion_mode)) {
      store_submi(xd, cm, search_state->best_submi, bsize);
    }
  ...
  *mbmi = it_ctx->base_mbmi;
  ...
  if (cpi->sf.inter_sf.prune_ref_mv_idx_search && is_comp_pred &&
      prune_ref_mv_idx_search(
          &cm->features, ref_mv_idx, search_state->best_ref_mv_idx[use_optflow],
          (*search_state->save_mv)[use_optflow][mbmi->pb_mv_precision], mbmi,
  ...
  int64_t skip_rd_local[2] = { env->skip_rd[0], env->skip_rd[1] };
  ...
  if (is_comp_pred && !is_joint_amvd_coding_mode(mbmi->mode, mbmi->use_amvd) &&
    ...
    const int not_best_mode = process_compound_inter_mode(
        ...
        &tmp_rate_mv, rd_stats, skip_rd_local, &skip_build_pred);
  ...
  ret_val = motion_mode_rd(
      ...
      *search_state->ref_best_rd, skip_rd_local, &tmp_rate_mv, env->orig_dst,
      env->best_est_rd, env->do_tx_search, env->inter_modes_info,
      env->top_motion_mode_model_rd ? env->top_motion_mode_model_rd[use_optflow]
                                    : NULL,
      0);
  ...
  if (ret_val != INT64_MAX) {
    env->skip_rd[0] = skip_rd_local[0];
    env->skip_rd[1] = skip_rd_local[1];
    int64_t tmp_rd = RDCOST(x->rdmult, rd_stats->rate, rd_stats->dist);

    if (is_pb_mv_prec_active &&
        tmp_rd < search_state->best_precision_rd_so_far[use_optflow]) {
      search_state->best_precision_so_far[use_optflow] = mbmi->pb_mv_precision;
      search_state->best_precision_dx_so_far[use_optflow] = precision_dx;
      search_state->best_precision_rd_so_far[use_optflow] = tmp_rd;
    }

    if (tmp_rd < (*search_state->mode_info)[bawp_flag][mbmi->pb_mv_precision]
                                           [use_optflow][ref_mv_idx_type]
                                               .rd) {
      // Only update mode_info if the new result is actually
      // better.
      (*search_state->mode_info)[bawp_flag][mbmi->pb_mv_precision][use_optflow]
                                [ref_mv_idx_type]
    ...
    if (tmp_rd < *search_state->ref_best_rd) {
      *search_state->ref_best_rd = tmp_rd;
      search_state->best_ref_mv_idx[use_optflow][0] = ref_mv_idx[0];
      search_state->best_ref_mv_idx[use_optflow][1] = ref_mv_idx[1];
    }
  }
  restore_dst_buf(xd, *env->orig_dst, num_planes);
}
...
static int64_t handle_inter_mode(
    ...
    int64_t top_motion_mode_model_rd[2][TOP_MOTION_MODE_MODEL_COUNT],
    PruneInfoFromTpl *inter_cost_info_from_tpl, int64_t *ref_frame_rd,
  ...
  for (int bawp = 0; bawp < BAWP_OPTION_CNT; bawp++) {
    for (int prec = MV_PRECISION_8_PEL; prec <= mbmi->max_mv_precision;
      ...
      for (int flat_idx = 0; flat_idx < total_ref_mv_idx; ++flat_idx) {
        int ref_mv_id_0 = flat_idx / ref_set[1];
        int ref_mv_id_1 = flat_idx % ref_set[1];
        const int idx = has_two_drls
                            ? ref_mv_id_1 * MAX_REF_MV_SEARCH + ref_mv_id_0
                            : ref_mv_id_0;
        for (int variant = 0; variant < 2; variant++) {
          mode_info[bawp][prec][variant][idx].full_search_mv.as_int =
              INVALID_MV;
          mode_info[bawp][prec][variant][idx].mv.as_int = INVALID_MV;
          mode_info[bawp][prec][variant][idx].rd = INT64_MAX;
          mode_info[bawp][prec][variant][idx].drl_cost = 0;
          mode_info[bawp][prec][variant][idx].rate_mv = 0;
          mode_info[bawp][prec][variant][idx].full_mv_rate = 0;
        }
      }
    }
  }
  ...
  int_mv save_mv[2][NUM_MV_PRECISIONS][MAX_REF_MV_SEARCH * MAX_REF_MV_SEARCH]
                [2];

  int best_ref_mv_idx[2][2] = { { -1, -1 }, { -1, -1 } };

  int max_optflow = 1;
  PREDICTION_MODE optflow_mode = MODE_INVALID;
  ...
  int skip_eval_mode[2] = { 0, 0 };
  int skip_eval_mode_motion[2] = { 0, 0 };
  int *orig_skip_motion_mode = sf_args->skip_motion_mode;

  sf_args->skip_motion_mode = &skip_eval_mode_motion[0];
  skip_eval_mode[0] = skip_inter_mode(cpi, x, bsize, ref_frame_rd, this_mode,
                                      mbmi->ref_frame, sf_args);
  if (max_optflow > 1) {
    mbmi->mode = optflow_mode;
    sf_args->skip_motion_mode = &skip_eval_mode_motion[1];
    skip_eval_mode[1] = skip_inter_mode(cpi, x, bsize, ref_frame_rd,
                                        optflow_mode, mbmi->ref_frame, sf_args);
    mbmi->mode = this_mode;
  }
  sf_args->skip_motion_mode = orig_skip_motion_mode;

  int skip_all = skip_eval_mode[0];
  if (max_optflow > 1) {
    skip_all &= skip_eval_mode[1];
  }
  if (skip_all) {
    av2_invalid_rd_stats(rd_stats);
    restore_dst_buf(xd, orig_dst, num_planes);
  ...
  for (int pb_mv_precision = mbmi->max_mv_precision;
    ...
    for (i = 0; i < MAX_REF_MV_SEARCH * MAX_REF_MV_SEARCH - 1; ++i) {
      save_mv[0][pb_mv_precision][i][0].as_int = INVALID_MV;
      save_mv[0][pb_mv_precision][i][1].as_int = INVALID_MV;
      save_mv[1][pb_mv_precision][i][0].as_int = INVALID_MV;
      save_mv[1][pb_mv_precision][i][1].as_int = INVALID_MV;
    }
  }

  int flex_mv_cost[NUM_MV_PRECISIONS] = { 0, 0, 0, 0, 0, 0, 0 };
  int idx_mask[BAWP_OPTION_CNT][NUM_MV_PRECISIONS][2] = { { { 0 } } };
  if (is_pb_mv_prec_active) {
    ...
    for (int precision_dx = precision_def->num_precisions - 1;
      ...
      for (int variant = 0; variant < max_optflow; variant++) {
        if (skip_eval_mode[variant]) continue;
        PREDICTION_MODE eval_mode = (variant == 0) ? this_mode : optflow_mode;
        mbmi->mode = eval_mode;
        idx_mask[0][pb_mv_precision][variant] =
            ref_mv_idx_to_search(cpi, x, rd_stats, args, ref_best_rd,
                                 mode_info[0][pb_mv_precision][variant], bsize,
        ...
        if (cm->features.enable_bawp &&
          ...
          for (int bawp = 1; bawp < BAWP_OPTION_CNT; bawp++) {
            ...
            idx_mask[bawp][pb_mv_precision][variant] = ref_mv_idx_to_search(
                cpi, x, rd_stats, args, ref_best_rd,
                mode_info[bawp][pb_mv_precision][variant], bsize, ref_set,
                flex_mv_cost[pb_mv_precision]);
          }
          mbmi->bawp_flag[0] = 0;
        }
      }
    ...
    for (int variant = 0; variant < max_optflow; variant++) {
      ...
      PREDICTION_MODE eval_mode = (variant == 0) ? this_mode : optflow_mode;
      mbmi->mode = eval_mode;
      idx_mask[0][mbmi->max_mv_precision][variant] = ref_mv_idx_to_search(
          cpi, x, rd_stats, args, ref_best_rd,
          mode_info[0][mbmi->max_mv_precision][variant], bsize, ref_set, 0);

      if (cm->features.enable_bawp &&
          av2_allow_bawp(cm, mbmi, xd->mi_row, xd->mi_col)) {
        for (int bawp = 1; bawp < BAWP_OPTION_CNT; bawp++) {
          mbmi->bawp_flag[0] = bawp;
          idx_mask[bawp][mbmi->max_mv_precision][variant] =
              ref_mv_idx_to_search(
                  cpi, x, rd_stats, args, ref_best_rd,
                  mode_info[bawp][mbmi->max_mv_precision][variant], bsize,
                  ref_set, 0);
        }
        mbmi->bawp_flag[0] = 0;
      }
  ...
  const int total_ref_mv_idx = ref_set[1] * ref_set[0];
  for (int flat_idx = 0; flat_idx < total_ref_mv_idx; ++flat_idx) {
    ref_mv_idx[0] = flat_idx / ref_set[1];
    ref_mv_idx[1] = flat_idx % ref_set[1];
    ...
    MvSubpelPrecision best_precision_so_far[2] = { mbmi->max_mv_precision,
                                                   mbmi->max_mv_precision };
    int64_t best_precision_rd_so_far[2] = { INT64_MAX, INT64_MAX };
    set_precision_set(cm, xd, mbmi, bsize, ref_mv_idx);
    set_most_probable_mv_precision(cm, mbmi, bsize);
    const PRECISION_SET *precision_def =
        &av2_mv_precision_sets[mbmi->mb_precision_set];
    int best_precision_dx_so_far[2] = { precision_def->num_precisions,
                                        precision_def->num_precisions };

    PredictorSearchState search_state;
    init_predictor_search_state(
        &search_state, &best_rd, &best_rd_stats, &best_rd_stats_y,
        &best_rd_stats_uv, &best_mbmi, best_submi, &best_xskip_txfm,
        best_blk_skip, best_tx_type_map, best_cctx_type_map,
        best_precision_so_far, best_precision_dx_so_far,
        best_precision_rd_so_far, best_cwp_costs, best_cwp_idxs, &ref_best_rd,
        best_ref_mv_idx, &mode_info, &save_mv);
    ...
    for (int precision_dx = precision_def->num_precisions - 1;
      ...
      if (!is_comp_pred) {
        ...
        int_mv cur_mv[2];
        if (mbmi->mode != WARPMV &&
            !build_cur_mv(cur_mv, this_mode, cm, x,
                          cpi->sf.inter_sf.skip_repeated_ref_mv)) {
          continue;
        }
        if (mbmi->mode == WARPMV) {
          cur_mv[0].as_int = 0;
        ...
        int bawp_eanbled = cm->features.enable_bawp && !mbmi->refinemv_flag &&
                           av2_allow_bawp(cm, mbmi, xd->mi_row, xd->mi_col);
        ...
        for (int b_idx = 0; b_idx < total_bawp_iters; b_idx++) {
          ...
          if (this_mode != WARPMV && !mbmi->refinemv_flag &&
              !mask_check_bit(idx_mask[bawp_flag][mbmi->pb_mv_precision][0],
                              ref_mv_idx_type)) {
            continue;
          }

          if (bawp_flag_uv == 0) {
            mode_info[bawp_flag][mbmi->pb_mv_precision][0][ref_mv_idx_type]
                .full_search_mv.as_int = INVALID_MV;
            mode_info[bawp_flag][mbmi->pb_mv_precision][0][ref_mv_idx_type]
                .mv.as_int = INVALID_MV;
            mode_info[bawp_flag][mbmi->pb_mv_precision][0][ref_mv_idx_type].rd =
                INT64_MAX;
            mode_info[bawp_flag][mbmi->pb_mv_precision][0][ref_mv_idx_type]
                .drl_cost = drl_cost;
          }
          ...
          if (mbmi->bawp_flag[0] >= 1) {
            ...
            mode_info[mbmi->bawp_flag[0]][mbmi->pb_mv_precision][0]
                     [ref_mv_idx_type]
                         .full_search_mv.as_int =
                mode_info[0][mbmi->pb_mv_precision][0][ref_mv_idx_type]
                    .full_search_mv.as_int;
            mode_info[mbmi->bawp_flag[0]][mbmi->pb_mv_precision][0]
                     [ref_mv_idx_type]
                         .full_mv_rate =
            ...
            if (have_newmv_in_inter_mode(this_mode)) {
              ...
              newmv_ret_val =
                  handle_newmv(cpi, x, bsize, cur_mv, &rate_mv, args,
                               mode_info[bawp_flag][mbmi->pb_mv_precision][0]);
          ...
          if (have_newmv_in_inter_mode(this_mode)) {
            ...
            if (skip_new_mv &&
                skip_repeated_newmv(
                    ...
                    &best_rd_stats_uv,
                    mode_info[bawp_flag][mbmi->pb_mv_precision][0], args,
                    drl_cost, refs, cur_mv, &best_rd, orig_dst, ref_mv_idx))
              continue;
          }

          const MB_MODE_INFO base_mbmi_single = *mbmi;
          PredictorIterationContext it_ctx;
          init_predictor_iteration_context(
              &it_ctx, bsize, 0, ref_mv_idx[0], ref_mv_idx[1], precision_dx,
              bawp_flag, ref_mv_idx_type, 0, NULL, this_mode, refs,
              flex_mv_cost, drl_cost, jmvd_scale_mode_cost, base_rate, cur_mv,
              rate_mv, base_mbmi_single, 0, num_planes,
              skip_eval_mode_motion[0]);
        ...
        mode_info[0][mbmi->pb_mv_precision][0][ref_mv_idx_type]
            .full_search_mv.as_int = INVALID_MV;
        mode_info[0][mbmi->pb_mv_precision][0][ref_mv_idx_type].mv.as_int =
            INVALID_MV;
        mode_info[0][mbmi->pb_mv_precision][0][ref_mv_idx_type].rd = INT64_MAX;
        mode_info[0][mbmi->pb_mv_precision][0][ref_mv_idx_type].drl_cost =
            drl_cost;

        if (this_mode != WARPMV && !mbmi->refinemv_flag &&
            !mask_check_bit(idx_mask[0][mbmi->pb_mv_precision][0],
                            ref_mv_idx_type)) {
          continue;
        }
        ...
        if (have_newmv_in_inter_mode(this_mode)) {
          ...
          newmv_ret_val = handle_newmv(cpi, x, bsize, cur_mv, &rate_mv, args,
                                       mode_info[0][mbmi->pb_mv_precision][0]);
        ...
        if (have_newmv_in_inter_mode(this_mode)) {
          ...
          for (int ref = 0; ref < 2; ref++) {
            ...
            if (single_mode == NEWMV || single_mode == WARP_NEWMV) {
              ...
              if (mbmi->pb_mv_precision < MV_PRECISION_HALF_PEL)
                lower_mv_precision(&ref_mv, mbmi->pb_mv_precision);
              av2_make_default_subpel_ms_params(
                  &ms_params, cpi, x, bsize, &ref_mv, pb_mv_precision, 0, NULL);
              if (!av2_is_subpelmv_in_range(&ms_params.mv_limits,
                                            cur_mv[ref].as_mv)) {
                mv_outlim = 1;
                break;
          ...
          if (skip_new_mv &&
              skip_repeated_newmv(
                  ...
                  mbmi->bawp_flag[0], mbmi->bawp_flag[1], &best_mbmi,
                  motion_mode_cand, &ref_best_rd, &best_rd_stats,
                  &best_rd_stats_y, &best_rd_stats_uv,
                  mode_info[0][mbmi->pb_mv_precision][0], args, drl_cost, refs,
                  cur_mv, &best_rd, orig_dst, ref_mv_idx))
            continue;
        }

        int_mv base_cur_mv[2];
        base_cur_mv[0] = cur_mv[0];
        base_cur_mv[1] = cur_mv[1];
        int base_rate_mv = rate_mv;

        const MB_MODE_INFO base_mbmi = *mbmi;

        // Declare PredictorIterationContext once for all compound blocks
        PredictorIterationContext it_ctx;
        ...
        init_predictor_iteration_context(
            &it_ctx, bsize, 0, ref_mv_idx[0], ref_mv_idx[1], precision_dx,
            bawp_flag, ref_mv_idx_type, 0, cwp_search_mask, this_mode, refs,
            flex_mv_cost, drl_cost, jmvd_scale_mode_cost, base_rate,
            base_cur_mv, base_rate_mv, base_mbmi, 0, num_planes,
            skip_eval_mode_motion[0]);
        ...
        // BLOCK 2: JMVD SCALES
        // -------------------------------------------------------------
        const MB_MODE_INFO base_mbmi_jmvd = *mbmi;
        for (int scale_index = 1; scale_index < jmvd_scaling_factor_num;
          ...
          int_mv cur_mv_scale[2];
          int rate_mv_scale = 0;
          cur_mv_scale[0] = base_cur_mv[0];
          cur_mv_scale[1] = base_cur_mv[1];
          ...
          start_timing(cpi, handle_newmv_time);
#endif
          newmv_ret_val = handle_newmv(cpi, x, bsize, cur_mv_scale, &rate_mv_scale, args,
                                       mode_info[0][mbmi->pb_mv_precision][0]);
#if CONFIG_COLLECT_COMPONENT_TIMING
          end_timing(cpi, handle_newmv_time);
#endif
          ...
          for (int ref = 0; ref < 2; ref++) {
            ...
            if (single_mode == NEWMV || single_mode == WARP_NEWMV) {
              ...
              if (mbmi->pb_mv_precision < MV_PRECISION_HALF_PEL)
                lower_mv_precision(&ref_mv, mbmi->pb_mv_precision);
              av2_make_default_subpel_ms_params(
                  &ms_params, cpi, x, bsize, &ref_mv, mbmi->pb_mv_precision, 0, NULL);
              if (!av2_is_subpelmv_in_range(&ms_params.mv_limits,
                                            cur_mv_scale[ref].as_mv)) {
                mv_outlim = 1;
                break;
              }
            }
          ...
          if (skip_new_mv &&
              skip_repeated_newmv(
                  ...
                  &best_rd_stats_y, &best_rd_stats_uv,
                  mode_info[0][mbmi->pb_mv_precision][0], args, drl_cost, refs,
                  cur_mv_scale, &best_rd, orig_dst, ref_mv_idx))
            continue;

          const int cur_jmvd_scale_cost =
              get_jmvd_scale_mode_cost(mbmi, mode_costs);

          init_predictor_iteration_context(
              &it_ctx, bsize, 0, ref_mv_idx[0], ref_mv_idx[1], precision_dx,
              bawp_flag, ref_mv_idx_type, scale_index, cwp_search_mask,
              this_mode, refs, flex_mv_cost, drl_cost, cur_jmvd_scale_cost,
              base_rate, cur_mv_scale, rate_mv_scale, *mbmi, 0,
              num_planes, skip_eval_mode_motion[0]);
        ...
        *mbmi = base_mbmi_jmvd;
        ...
        // BLOCK 3: CWP MODES
        // -------------------------------------------------------------
        const MB_MODE_INFO base_mbmi_cwp = *mbmi;
        for (int cwp_search_idx = 1; cwp_search_idx < cwp_loop_num;
             cwp_search_idx++) {
          mbmi->cwp_idx = cwp_weighting_factor[same_side][cwp_search_idx];
          if (is_cwp_allowed(mbmi) && mbmi->cwp_idx != -1 &&
              cwp_search_mask[cwp_search_idx] != 0) {
            int_mv cur_mv_cwp[2];
            int rate_mv_cwp = 0;
            cur_mv_cwp[0] = base_cur_mv[0];
            cur_mv_cwp[1] = base_cur_mv[1];

            if (have_newmv_in_inter_mode(this_mode)) {
#if CONFIG_COLLECT_COMPONENT_TIMING
              start_timing(cpi, handle_newmv_time);
#endif
              newmv_ret_val = handle_newmv(cpi, x, bsize, cur_mv_cwp, &rate_mv_cwp, args,
                                           mode_info[0][mbmi->pb_mv_precision][0]);
#if CONFIG_COLLECT_COMPONENT_TIMING
              end_timing(cpi, handle_newmv_time);
#endif
              ...
              if (skip_new_mv &&
                  skip_repeated_newmv(
                      ...
                      &best_rd_stats_y, &best_rd_stats_uv,
                      mode_info[0][mbmi->pb_mv_precision][0], args, drl_cost, refs,
                      cur_mv_cwp, &best_rd, orig_dst, ref_mv_idx))
                continue;
            } else {
              rate_mv_cwp = base_rate_mv;
            }
            ...
            init_predictor_iteration_context(
                ...
                bawp_flag, ref_mv_idx_type, 0, cwp_search_mask, this_mode, refs,
                flex_mv_cost, drl_cost, jmvd_scale_mode_cost, base_rate,
                cur_mv_cwp, rate_mv_cwp, *mbmi, 0, num_planes,
                skip_eval_mode_motion[0]);
        ...
        *mbmi = base_mbmi_cwp;
        ...
        // -------------------------------------------------------------
        // BLOCK 4: OPTICAL FLOW AND REFINEMV (DMVR)
        // -------------------------------------------------------------
        const MB_MODE_INFO base_mbmi_opfl = *mbmi;
        for (int use_optflow = 0; use_optflow < max_optflow; ++use_optflow) {
          PREDICTION_MODE eval_mode = use_optflow ? optflow_mode : this_mode;
          mbmi->mode = eval_mode;

          if (skip_eval_mode[use_optflow]) continue;
          if (!has_second_drl(mbmi) && ref_mv_idx[1] > 0) continue;

          if (use_optflow) {
            if (!opfl_allowed_cur_refs_bsize(cm, xd, mbmi) ||
          ...
          const int max_scale =
              (use_optflow && is_joint_mvd_coding_mode(eval_mode))
                  ? jmvd_scaling_factor_num
                  : 1;

          const MB_MODE_INFO base_mbmi_scale = *mbmi;
          for (int scale_index = 0; scale_index < max_scale; ++scale_index) {
            ...
            if (use_optflow) {
              ...
              mbmi->ref_mv_idx[0] = ref_mv_idx[0];
              set_mv_precision(mbmi, mbmi->max_mv_precision);
              ref_mv_idx_type = av2_ref_mv_idx_type(mbmi, ref_mv_idx);

              // -------------------------------------------------------------
              // Initialize mode_info elements properly for optical flow modes
              // to prevent reading uninitialized memory state during sub-pel
              // search checks.
              // -------------------------------------------------------------
              mode_info[0][mbmi->pb_mv_precision][use_optflow][ref_mv_idx_type]
                  .full_search_mv.as_int = INVALID_MV;
              mode_info[0][mbmi->pb_mv_precision][use_optflow][ref_mv_idx_type]
                  .mv.as_int = INVALID_MV;
              mode_info[0][mbmi->pb_mv_precision][use_optflow][ref_mv_idx_type]
              ...
              int64_t cur_ref_best_rd = ref_best_rd;
              if (mbmi->mode != WARPMV && prune_modes_based_on_tpl &&
                  !ref_match_found_in_above_nb && !ref_match_found_in_left_nb &&
                  (cur_ref_best_rd != INT64_MAX)) {
                if (prune_modes_based_on_tpl_stats(
                        &cm->features, inter_cost_info_from_tpl, refs,
                        ref_mv_idx[0], eval_mode,
                        cpi->sf.inter_sf.prune_inter_modes_based_on_tpl))
                  continue;
              }
              ...
              if (is_pb_mv_prec_active) {
                if (cpi->sf.flexmv_sf.terminate_early_4_pel_precision &&
                    pb_mv_precision < MV_PRECISION_FOUR_PEL &&
                    best_precision_so_far[use_optflow] >= MV_PRECISION_QTR_PEL)
                  continue;
                if (prune_curr_mv_precision_eval(
                        cpi->sf.flexmv_sf
                            .prune_mv_prec_using_best_mv_prec_so_far,
                        precision_dx, best_precision_dx_so_far[use_optflow]))
                  continue;
              }
              ...
              int_mv cur_mv_opfl[2];
              if (mbmi->mode != WARPMV &&
                  !build_cur_mv(cur_mv_opfl, eval_mode, cm, x, 0)) {
                continue;
              }

              if (have_newmv_in_inter_mode(eval_mode)) {
#if CONFIG_COLLECT_COMPONENT_TIMING
                start_timing(cpi, handle_newmv_time);
#endif
                newmv_ret_val = handle_newmv(
                    cpi, x, bsize, cur_mv_opfl, &rate_mv, args,
                    mode_info[0][mbmi->pb_mv_precision][use_optflow]);
              ...
              if (have_newmv_in_inter_mode(eval_mode)) {
                ...
                if (skip_new_mv &&
                    skip_repeated_newmv(
                        cpi, x, bsize, do_tx_search, eval_mode,
                        mbmi->pb_mv_precision, mbmi->bawp_flag[0],
                        mbmi->bawp_flag[1], &best_mbmi, motion_mode_cand,
                        &ref_best_rd, &best_rd_stats, &best_rd_stats_y,
                        &best_rd_stats_uv,
                        mode_info[0][mbmi->pb_mv_precision][use_optflow], args,
                        drl_cost, refs, cur_mv_opfl, &best_rd, orig_dst,
                        ref_mv_idx))
                  continue;
              }

              cur_mv_to_use[0] = cur_mv_opfl[0];
              cur_mv_to_use[1] = cur_mv_opfl[1];
              rate_mv_to_use = rate_mv;
            } else {
              cur_mv_to_use[0] = base_cur_mv[0];
              cur_mv_to_use[1] = base_cur_mv[1];
              rate_mv_to_use = base_rate_mv;
            }

            // -------------------------------------------------------------
            // Recalculate prediction mode cost and base_rate dynamically since
            // eval_mode can change inside the use_optflow loop, altering rate
            // estimates.
            // -------------------------------------------------------------
            const int pred_mode_cost = cost_prediction_mode(
                mode_costs, eval_mode, cm, mbmi, xd, mode_ctx);
            const int base_rate_dynamic = args->ref_frame_cost +
                                          args->single_comp_cost +
                                          pred_mode_cost;

            const int jmvd_scale_mode_cost_dynamic =
                get_jmvd_scale_mode_cost(mbmi, mode_costs);

            const int start_refinemv = use_optflow ? 0 : 1;
            for (int refinemv_loop = start_refinemv;
                 refinemv_loop < REFINEMV_NUM_MODES; refinemv_loop++) {
              init_predictor_iteration_context(
                  &it_ctx, bsize, use_optflow, ref_mv_idx[0], ref_mv_idx[1],
                  precision_dx, bawp_flag, ref_mv_idx_type, scale_index,
                  cwp_search_mask, eval_mode, refs, flex_mv_cost, drl_cost,
                  jmvd_scale_mode_cost_dynamic, base_rate_dynamic,
                  cur_mv_to_use, rate_mv_to_use, *mbmi, refinemv_loop,
                  num_planes, skip_eval_mode_motion[use_optflow]);

              evaluate_inter_predictor(cpi, tile_data, x, &env, &it_ctx,
                                       &search_state);
            }
          }
          *mbmi = base_mbmi_scale;
        }
        *mbmi = base_mbmi_opfl;
      }
    }
...
void av2_rd_pick_inter_mode_sb(struct AV2_COMP *cpi,
  ...
  for (PREDICTION_MODE this_mode = 0; this_mode < MB_MODE_COUNT; ++this_mode) {
    // Optical flow modes are searched with a separate loop in handle_inter_mode
    if (this_mode >= COMP_OPTFLOW_MODE_START && this_mode < COMP_INTER_MODE_END)
      continue;
    ...
    if (enable_tx_prune) {
      for (int k = 0; k < TOP_MOTION_MODE_MODEL_COUNT; k++) {
        top_motion_mode_model_rd[0][k] = INT64_MAX;
        top_motion_mode_model_rd[1][k] = INT64_MAX;
      }
    }
    ...
    for (int pair_idx = 0; pair_idx < num_pairs; ++pair_idx) {
      ...
      for (int use_amvd_mode = 0; use_amvd_mode < num_amvd_modes;
        ...
        int64_t this_rd = handle_inter_mode(
            ...
            &inter_cost_info_from_tpl, ref_frame_rd, &sf_args);
